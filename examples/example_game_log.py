...
def main() -> None:
    ...
    engine.apply_action(ActionRequest(player_id="p2", action=PlayerAction.FOLD))
    engine.apply_action(ActionRequest(player_id="p3", action=PlayerAction.CALL))
    engine.deal_turn()
    engine.apply_action(ActionRequest(player_id="p1", action=PlayerAction.CHECK))
    engine.apply_action(ActionRequest(player_id="p3", action=PlayerAction.ALL_IN))
    engine.deal_river()
